  ...
  return function () {
    ...
    1: [function (require, module, exports) {
      ...
      var Box2D = /** @class */function () {
        ...
        Box2D.prototype.step = function (dt, velocityIteration, positionIteration) {
          if (velocityIteration === void 0) {
            velocityIteration = 10;
          }
          if (positionIteration === void 0) {
        ...
        Box2D.prototype.isContact = function (body1, body2, contact) {
          ...
          if (body1.b2Body.GetUserData() === bodyA && body2.b2Body.GetUserData() === bodyB || body1.b2Body.GetUserData() === bodyB && body2.b2Body.GetUserData() === bodyA) {
            return true;
          }
          return false;
        };
    ...
    7: [function (require, module, exports) {
      ...
      function main(param) {
        ...
        var randomGenerate2 = [];
        for (var i = 0; i < 16; i++) {
          randomGenerate2[i] = Math.floor(randomGenerate * Math.pow(10, i + 1)) % 10;
        }
        ...
        function createSceneA2() {
          ...
          scene.onLoad.add(function () {
            ...
            var floorColor = colors.floorColor;
            ...
            for (var i = 0; i < totalSegments; i++) {
              var d = amplitude * Math.sin(frequency * (i + y));
              var prevFloor = objects[i];
              if ((d > 0 && angle < 0 || d < 0 && angle > 0) && count > 10) {
                amplitude = 10 + randomGenerate2[randomCount] * 3;
                frequency = 0.1 + randomGenerate2[randomCount2] * 0.025;
                var d1 = amplitude * Math.sin(frequency * i);
                for (var j = 0; j < 100; j += 0.5) {
                  var d2 = amplitude * Math.sin(frequency * (i + j));
                  if (d1 * d2 <= 0) {
                    y = j;
                    break;
                  }
                ...
                var d = amplitude * Math.sin(frequency * (i + y));
                count = 0;
                if (randomGenerate2[randomCount] < 5) {
                  ab *= -1;
                }
                if (randomCount == 15) {
                  randomCount = 0;
                ...
                //console.log(randomCount,randomCount2);
              }
              angle = d;
              count++;
              ...
              if (d * ab <= prevFloor.angle + 0.5) {
                var floor = new g.FilledRect({
                  scene: scene,
                  x: prevFloor.x + 50 * Math.cos(prevFloor.angle * Math.PI / 180),
                  y: prevFloor.y + 50 * Math.sin(prevFloor.angle * Math.PI / 180),
                  anchorX: 0.5,
                  angle: (prevFloor.angle + d * ab) / 2,
                  cssColor: floorColor,
                  width: 100,
                  height: 1000,
                  parent: scene
            ...
            var ki3 = new g.Sprite({
              scene: scene,
              src: scene.asset.getImageById("ki"),
              x: 5000 + objects[totalSegments].x + 50 * Math.cos(objects[totalSegments].angle * Math.PI / 180),
              y: objects[totalSegments].y + 50 * Math.sin(objects[totalSegments].angle * Math.PI / 180),
              anchorX: 1,
              anchorY: 1,
              scaleX: 0.8,
              scaleY: 0.8,
            ...
            if (objects[objects.length - 1].angle > -0.5) {
              var floor = new g.FilledRect({
                scene: scene,
                x: objects[totalSegments].x + 50 * Math.cos(objects[totalSegments].angle * Math.PI / 180),
                y: objects[totalSegments].y + 50 * Math.sin(objects[totalSegments].angle * Math.PI / 180),
                anchorX: 0.5,
            ...
            var rect1 = new g.Sprite({
              scene: scene,
              src: scene.asset.getImageById("car"),
              x: 200,
              y: 320,
              anchorY: 1,
              parent: scene
            ...
            box2d.createBody(ki2, dynamicDef, box2d.createFixtureDef({
              density: 15,
              //密度0.1
              friction: 1,
              //摩擦
              restitution: 0,
              // 反発係数
              shape: box2d.createRectShape(ki2.width * 0.8, ki2.height * 0.8),
              filter: {
                categoryBits: 4,
                maskBits: 3
              }
            ...
            // f
            var f2 = box2d.createBody(
            //軸
            f, dynamicDef, box2d.createFixtureDef({
              density: 10,
              //密度
              friction: 0.5,
              //摩擦
              restitution: 0.2,
              // 反発係数
              shape: box2d.createRectShape(f.width, f.height),
              filter: {
                categoryBits: 2,
                maskBits: 5
              }
            ...
            // r
            var r2 = box2d.createBody(
            //軸
            r, dynamicDef, box2d.createFixtureDef({
              density: 10,
              //密度1.5
              friction: 0.5,
              //摩擦
              restitution: 0.2,
              // 反発係数
              shape: box2d.createRectShape(r.width, r.height),
              filter: {
                categoryBits: 2,
                maskBits: 5
              }
            ...
            var axis = new b2.Box2DWeb.Common.Math.b2Vec2(0, -1);
            // ジョイントの定義と初期化
            var def1 = new b2.Box2DWeb.Dynamics.Joints.b2PrismaticJointDef();
            def1.Initialize(a.b2Body, r2.b2Body, box2d.vec2(r.x, r.y), axis);
            def1.maxMotorForce = 100;
            def1.motorSpeed = 10;
            def1.enableMotor = true;
            def1.lowerTranslation = 0;
            def1.upperTranslation = 15 / 50;
            def1.enableLimit = true;
            var joint1 = box2d.world.CreateJoint(def1);
            var def2 = new b2.Box2DWeb.Dynamics.Joints.b2PrismaticJointDef();
            def2.Initialize(a.b2Body, f2.b2Body, box2d.vec2(f.x, f.y), axis);
            def2.maxMotorForce = 100;
            def2.motorSpeed = 10;
            def2.enableMotor = true;
            def2.lowerTranslation = 0;
            def2.upperTranslation = 15 / 50;
            def2.enableLimit = true;
            var joint2 = box2d.world.CreateJoint(def2);
            ...
            def4.Initialize(r2.b2Body, b.b2Body, box2d.vec2(tire1.x, tire1.y));
            def4.maxMotorTorque = 3;
            def4.motorSpeed = 0;
            def4.enableMotor = true;
            var def5 = new b2.Box2DWeb.Dynamics.Joints.b2RevoluteJointDef();
            def5.Initialize(f2.b2Body, c.b2Body, box2d.vec2(tire2.x, tire2.y));
            def5.maxMotorTorque = 3;
            def5.motorSpeed = 0;
            ...
            function off() {
              L = false;
              R = false;
              joint4.SetMaxMotorTorque(1);
              joint4.SetMotorSpeed(0);
              joint5.SetMaxMotorTorque(1);
              joint5.SetMotorSpeed(0);
              lamp2.hide();
              re = false;
              fo = false;
            }
            ...
            var scoreLabel = new g.Label({
              scene: scene,
              x: 950,
              text: scoreText + g.game.vars.gameState.score,
              font: font,
              fontSize: font.size,
              textColor: textColor,
              parent: E
            ...
            function cameraXY() {
              ...
              camera.y = (camera.y * 3 + rect1.y - 400) / 4;
              camera.modified();
              E.x = camera.x;
              E.y = camera.y;
              E.modified();
            ...
            scene.onUpdate.add(function () {
              if (!reset && !isTitle) {
                ...
                joint1.SetMotorSpeed(-0.1 * joint1.GetJointTranslation() * 50);
                joint2.SetMotorSpeed(-0.1 * joint2.GetJointTranslation() * 50);
                if (joint1.GetJointTranslation() > 0.25 || joint2.GetJointTranslation() > 0.25) {
                  if (!hit && fo) {
                    hit = true;
                    seTime = 0;
                    scene.asset.getAudioById("on").stop();
                ...
                if (time <= 0) {
                  ...
                  if (g.game.vars.gameState.score < score) {
                    ...
                    scoreLabel.text = scoreText + g.game.vars.gameState.score;
            ...
            scene.setInterval(function () {
              if (rect1.x < floorEnd.x && rect1.y < 3000) {
                ...
                if ((rect1.angle < -1800 || rect1.angle > 1800) && rectAngle < 2) {
        ...
        function createSceneA3() {
          ...
          scene.onLoad.add(function () {
            ...
            for (var i = 0; i < totalSegments; i++) {
              var d = amplitude * Math.sin(frequency * (i + y));
              var prevFloor = objects[i];
              if ((d > 0 && angle < 0 || d < 0 && angle > 0) && count > 50) {
                amplitude = 5 + randomGenerate2[randomCount] * 1;
                frequency = 0.1 + randomGenerate2[randomCount2] * 0.1;
                var d1 = amplitude * Math.sin(frequency * i);
                for (var j = 0; j < 100; j += 0.5) {
                  var d2 = amplitude * Math.sin(frequency * (i + j));
                  if (d1 * d2 <= 0) {
                ...
                var d = amplitude * Math.sin(frequency * (i + y));
                count = 0;
                if (randomGenerate2[randomCount] < 3) {
                  ab *= -1;
                }
                //console.log(randomCount,randomCount2);
              }
            ...
            box2d.createBody(sabo2, staticDef, sabo2TopDef);
            //var endX=objects[totalSegments].x + (50 * Math.cos(objects[totalSegments].angle * Math.PI / 180))
            //if(objects[totalSegments].angle>-0.5){endX-=1;}
            if (objects[objects.length - 1].angle > -0.5) {
              var floor = new g.FilledRect({
            ...
            resetLabel.onPointDown.add(function () {
              if (time > 0 && !isTitle) {
                ...
                camera.x = -200; //-180
                camera.y = -105; //100
                g.game.replaceScene(createSceneA3());
              }
            });
            function cameraXY() {
              camera.x = (camera.x * 3 + rect1.x - 500) / 4;
            ...
            scene.onUpdate.add(function () {
              if (!reset && !isTitle) {
                ...
                box2d.step(1 / g.game.fps);
                if (joint1.GetJointTranslation() > trans1 + 0.02 || joint2.GetJointTranslation() > trans2 + 0.02) {
                  if (!hit && fo) {
                    hit = true;
                    seTime = 0;
                    scene.asset.getAudioById("on").stop();
                ...
                } else if (joint1.GetJointTranslation() < trans1 - 0.02 || joint2.GetJointTranslation() < trans2 - 0.02) {
                  hit = false;
                }
                trans1 = joint1.GetJointTranslation();
                trans2 = joint2.GetJointTranslation();
                joint1.SetMotorSpeed(-0.1 * trans1 * 50);
        ...
        if (map <= 77) {
          ...
          g.game.pushScene(createSceneA3()); //砂漠コース
        }
      }
      exports.main = main;
    }, {
      "@akashic-extension/akashic-box2d": 3
    }]
  }, {}, [6])(6);
});
