      ...
      function main(param) {
        ...
        scene.onLoad.add(function () {
          ...
          function generateHeart(x, y, color) {
            ...
            heart.onUpdate.add(function (e) {
              // 毎フレームカウンタを追加
              heart.tag.counter++;
              if (heart.tag.counter > 100) {
                // カウンタが100を超えていたら削除する
                box2d.removeBody(blockBody);
                heart.destroy();
              } else if (heart.tag.counter > 70) {
                // カウンタが50を超えていたら半透明にしていく
                heart.opacity = (100 - heart.tag.counter) / 30;
                // このエンティティが変更されたという通知
                heart.modified();
              }
            });
            var blockBody = box2d.createBody(heart, dynamicDef, box2d.createFixtureDef({
              density: 0.5,
              //密度
              friction: 0.5,
              //摩擦
              restitution: 0.2,
              //反発係数
              shape: Math.abs(heart.width - heart.height) < 5 ? box2d.createCircleShape(heart.width) : box2d.createRectShape(heart.width, heart.height)
            }));
          }
          ...
          container.onPointDown.add(function (e) {
          ...
          function createTool(assetId, y) {
            ...
            tool.invalidate();
            // このツールを選択した結果を反映
            tool.onPointDown.add(function (e) {
          ...
          panelSwitch.onPointDown.add(function (e) {
